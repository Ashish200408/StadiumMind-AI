...
export function EnvironmentalAlerts() {
  const detections = useSustainabilityStore((state) => state.detections);
  ...
  return (
    <Card
      ...
      role="region"
      aria-label="Environmental Alerts"
    >
      <h3 className="text-lg font-semibold text-white mb-4" id="env-alerts-heading">
        Environmental Alerts
      </h3>
      {detections.length === 0 ? (
        <p className="text-gray-400 text-sm">No active environmental alerts.</p>
      ) : (
        <ul
          className="space-y-3 max-h-[400px] overflow-y-auto pr-2"
          aria-labelledby="env-alerts-heading"
        >
          {detections.map((det) => (
            <li
              key={det.id}
